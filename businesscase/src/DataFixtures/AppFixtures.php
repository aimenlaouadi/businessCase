...
class AppFixtures extends Fixture
{
    public function load(ObjectManager $manager): void
    {
        ...
        $user1->setPassword(password_hash('test1', PASSWORD_BCRYPT)); // Hachage du mot de passe
        $manager->persist($user1);

        $user2 = new Users();
        $user2->setUsername('Admin');
        $user2->setPassword(password_hash('test2', PASSWORD_BCRYPT)); // Hachage du mot de passe
        $user2->setRoles(['Role_ADMIN']);
        $manager->persist($user2);

        $user3 = new Users();
        $user3->setUsername('SuperAdmin');
        $user3->setPassword(password_hash('test3', PASSWORD_BCRYPT)); // Hachage du mot de passe
